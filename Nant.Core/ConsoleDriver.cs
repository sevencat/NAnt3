    ...
    public class ConsoleDriver
    {
        ...
        public static int Main(string[] args)
        {
            ...
            try
            {
                ...
                if (cmdlineOptions.BuildFile != null)
                {
                    if (project != null)
                    {
                        Console.WriteLine(string.Format(CultureInfo.InvariantCulture,
                            "Buildfile has already been loaded! Using new value '{0}'; discarding old project file '{1}'",
                            cmdlineOptions.BuildFile, project.BuildFileUri));
                        // insert empty line
                        Console.WriteLine();
                    }
                ...
                {
                    project = new Project(
                        GetBuildFileName(Environment.CurrentDirectory, null, cmdlineOptions.FindInParent),
                        projectThreshold, cmdlineOptions.IndentationLevel);
                }
        ...
        public static string GetBuildFileName(string directory, string searchPattern, bool findInParent)
        {
            ...
            else
            {
                ...
                else if (files.Length == 0 && findInParent)
                {
                    // recurse up the tree
        ...
        private static void LoadExtensionAssemblies(StringCollection extensionAssemblies, Project project)
        {
            ...
            loadTasks.Threshold = (project.Threshold == Level.Debug) ? Level.Debug : Level.Warning;
        ...
        private static void AddBuildListeners(CommandLineOptions cmdlineOptions, Project project)
        {
            ...
            if (cmdlineOptions.LogFile != null)
            {
                try
                {
                    outputWriter = new StreamWriter(new FileStream(cmdlineOptions.LogFile.FullName, FileMode.Create,
