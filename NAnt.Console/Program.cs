    ...
    public class ConsoleStub
    {
        ...
        public static int Main(string[] args)
        {
            ...
            if (nantShadowCopyFilesSetting != null && bool.Parse(nantShadowCopyFilesSetting) == true)
            {
                ...
                {
                    System.Console.WriteLine("Failed to create: {0}. Using default CachePath." + e.ToString(),
                        cachePath);
                }
                finally
                {
        ...
        private static string GetRelativePath(string basePath, string absolutePath)
        {
            ...
            if ((int)Environment.OSVersion.Platform != 128)
            {
                ...
                if (string.Compare(Path.GetPathRoot(fullBasePath), Path.GetPathRoot(fullAbsolutePath),
                        caseInsensitive) != 0)
                {
            ...
            int i = 0;
            while (i < absoluteLen && i < baseLen &&
                   string.Compare(fullBasePath[i].ToString(), fullAbsolutePath[i].ToString(), caseInsensitive) == 0)
            {
                i++;
            }

            if (i == baseLen && (fullAbsolutePath[i] == Path.DirectorySeparatorChar ||
        ...
        private class HelperArguments : MarshalByRefObject
        {
            ...
            public void CallConsoleRunner()
            {
                ...
                foreach (MethodInfo methodInfo in consoleDriverType.GetMethods(
        ...
        private class Framework
        {
            ...
            public static Framework GetRuntimeFramework()
            {
                ...
                XmlElement frameworkNode = (XmlElement)nantNode.SelectSingleNode("frameworks/platform[@name='" +
                    Platform + "']/framework[@family='" + FrameworkFamily + "' and @clrversion='" +
